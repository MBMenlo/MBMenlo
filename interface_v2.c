...
//PC sends address through TX (GPIO1:0x4200_0000), the register file will reply with the ADDRESS to the RX port(GPIO2: 0x4200_0008). Once it's received the PC verifies that is the same sent and start sending the data to store in the corresponding register.  


int main(int argc, char **argv)
{
int fd, mode_sel, kp, ki, kd, mux_ch1, mux_ch2, PSR = 14, DSR =10, ISR=24, kpx, kix = 0, kdx , PSRx = 24, ISRx = 0, DSRx= 10;
...
uint32_t pid1addr = 1, pid2addr = 2, muxaddr = 3, offsetaddr = 4, dith1addr = 5, dith2addr = 6, pid_g_addr = 9, pid3addr = 10, pid4addr = 11, pid_g_1_addr = 12, dith3addr = 13, dith4addr = 14, off1addr = 15, START = 16843009;
unsigned repeat = 1;
void *cfg;							
char *name = "/dev/mem";
...
printf("What parameter do you want to set?\n PID : 1, MUX_OUT: 2, OFFSET: 3, DITHER: 4, PID_GAIN: 5, PD_EXT_SET: 6, PD_EXT_GAIN: 7, OFFSET_PID_CH1: 8\n");
scanf("%d", &mode_sel);
...
if (mode_sel == 1)
	...
	*((uint32_t *)(cfg + 0)) = offsetaddr;
	...
	if (*((uint32_t *)(cfg + 8)) == offsetaddr) 		//Address received correctly
		{ 
				*((uint32_t *)(cfg + 0)) = ((uint32_t)(off_ch2_d)<<16) + (uint16_t)(off_ch1_d);
				sleep(wait_time);
